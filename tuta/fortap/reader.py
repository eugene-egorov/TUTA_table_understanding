...
class SheetReader(Reader):
    ...
    def result_from_file(self, json_path):
        ...
        try:
            ...
            left_position_list = self.read_header(table["LeftTreeRoot"], merged_regions, row_number, column_number,
                                                  False)
        ...
        # Step 4: Dictionary of coord to formulas
        table_range = table['RangeAddress']
        formula_dict = {}
        for i in range(len(cell_matrix)):
            for j in range(len(cell_matrix[0])):
                ...
                if cell['A1'] == '':
                    continue
                if 'INVALID' in cell['FormulaType']:
                    continue
                if 'INTABLE' in cell['FormulaType']:
                    formula_dict[(i, j)] = {}
                    formula_dict[(i, j)]['A1'] = cell['A1']
                    formula_dict[(i, j)]['R1'] = cell['R1']
                    formula_dict[(i, j)]['FormulaType'] = cell['FormulaType']
                    formula_dict[(i, j)]['FormulaNodes'] = cell['FormulaNodes']
                    try:
                        sr_dict = prepare_sr(formula_info=formula_dict[(i, j)], table_range=table_range)
                        nr_dict = prepare_nr(formula_info=formula_dict[(i, j)], table_range=table_range)
                        formula_dict[(i, j)]['sr_dict'] = sr_dict
                        formula_dict[(i, j)]['nr_dict'] = nr_dict
                        tokens, types = convert_to_prefix(cell['A1'], cell['FormulaNodes'])
                        formula_dict[(i, j)]['FormulaTokens'] = tokens
                        formula_dict[(i, j)]['FormulaTokenTypes'] = types
                    except Exception as e:
                        # print(e)
                        del formula_dict[(i, j)]
                        continue

        if len(formula_dict) == 0:  # no formula means the table is meaningless
            return None

        meta_data = dict()
        meta_data['file_path'] = table['BlobName']
        meta_data['sheet_name'] = table['SheetName']
        meta_data['storage_account'] = table['StorageAccount']
        meta_data['language'] = table['Language']

        return string_matrix, (top_position_list, left_position_list), (header_rows, header_columns), format_matrix, \
               table_range, formula_dict, json_path, meta_data
